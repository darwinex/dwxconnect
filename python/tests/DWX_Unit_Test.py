...
class TestDWXConnect(unittest.TestCase):
    ...
    def open_multiple_orders(self):
        ...
        for i in range(self.num_open_orders):
            self.dwx.open_order(symbol=self.symbol, order_type='buy', 
                                ...
                                magic=self.magic_number, comment='', expriation=0)
    ...
    def test_try_read_file(self):
        result = self.dwx.try_read_file(join(self.MT4_directory_path, 'DWX', 'DWX_Orders.txt'))
    ...
    def test_open_modify_close_order(self):
        ...
        if not self.close_all_orders():
            self.fail('Could not close orders after test_open_modify_close_order().')
    ...
    def test_close_orders_by_symbol(self):
        ...
        if not self.open_multiple_orders():
            self.fail('Could not open all orders in test_close_orders_by_symbol().')
    ...
    def test_close_orders_by_magic(self):
        ...
        if not self.open_multiple_orders():
            self.fail('Could not open all orders in test_close_orders_by_magic().')
    ...
    def test_get_historic_data(self):
        ...
        while now < start_time + timedelta(seconds=5):
            ...
            now = datetime.utcnow()
            try:
                historic_data = self.dwx.historic_data[self.symbol + '_' + time_frame]
                # print(historic_data)
                break
            except:
                historic_data = None
